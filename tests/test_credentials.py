...
from decisioncenter_mcp_server.MCPServer import create_credentials
...
def test_create_credentials_single_ssl_cert_path(tmp_path):
    ...
    assert cred.cacert == str(cert_file)
...
def test_create_credentials_multi_ssl_cert_path_comma(tmp_path):
    ...
    assert cred.cacert != str(f_a)
    assert os.path.isfile(cred.cacert)

    merged_content = open(cred.cacert).read()
    assert "certA" in merged_content
    assert "certB" in merged_content
    os.unlink(cred.cacert)
